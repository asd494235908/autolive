...
> V2.90 接线状态（2026-08-21）：桌面主窗口已接入控制面启动门禁。启动时读取本地设备标识并尝试 Refresh；无有效会话进入登录页，登录后未激活进入激活页，设备激活后才进入本地播放工作区。Refresh Token 只通过 Tauri `keyring` 命令保存，Access Token 只存在 React 内存；服务端 `APP_SECRET_ENCRYPTION_KEY` 和模型 API Key 不进入桌面端。登录后的设备运行摘要按固定周期发送 Heartbeat，瞬态网络失败只保留一条有界 outbox。Windows 原生安装、真实 HTTPS/DNS/TLS、钥匙串不可用实机提示和断网恢复仍需发布验收。
...
控制面请求使用 Tauri 官方 HTTP 插件，开发模式在未配置环境变量时仅回退到 `http://127.0.0.1:18090`，生产构建必须显式设置 `VITE_CONTROL_PLANE_BASE_URL=https://<实际域名>`；缺少生产地址时客户端直接拒绝启动，避免把私网 HTTP 地址固化进发布包。仓库 capability 仅保留开发回环 HTTP 和明确的 `https://admin.example.com/**` 模板 origin，禁止本地旧地址和任意 URL 通配权限。替换真实域名后必须从仓库根目录运行 `node tools/check-https-publish.mjs --env-file deploy/.env.example --reverse-proxy deploy/reverse-proxy/nginx.conf --capability desktop/src-tauri/capabilities/default.json --admin-nginx admin-web/nginx.conf` 并重新打包。桌面端 Access Token 只在 React 运行时内存中存在；Refresh Token 使用成熟 `keyring` 库经 Tauri 命令写入系统 Keychain/Credential Manager，登录和轮换后更新，启动时读取并刷新，退出或刷新失效时删除。钥匙串不可用时不阻断本次登录，但明确提示重启后需重新登录。
