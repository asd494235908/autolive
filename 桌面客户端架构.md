...
- 预热读取速率分两类：首次启动、恢复和跳转后的 CatchUp 使用受控 `-readrate max(2, playback_speed + 1) -readrate_initial_burst 0.5` 追上当前画面；已知未来切换点的 Scheduled 候选使用 `-re`，seek 到目标前 250ms 并只生成固定 750ms PCM 窗口。两类任务都受 5 秒准备预算和代际取消约束，失败时保持旧轨。
...
- 开启或重新配置 PortAudio 时先启动静音暂停态 callback，并为首轨原子预填 50ms。正常播放只维护独立的 50ms 目标水位；周期换轨必须同时取得旧/新轨 30ms 样本和新轨额外 100ms 淡化后尾部（候选总余量 130ms），一次性把完整 30ms 双向线性 ramp 追加到环缓后才切换 current。普通换轨不清空已有环缓，失败或候选过期继续旧轨。
- 应用侧 SPSC 内存容量默认 1024 KiB、允许 128～2048 KiB，仅决定可分配容量，不控制 256-frame 硬件 callback，也不等于播放延迟。输出线程按 50ms 水位逐步供给，不能让 FFmpeg 填满整个物理环缓；SPSC 写入支持跨越环尾。
...
- 输出线程以 callback 已消费 frame、实际采样率和 `PaStreamInfo.outputLatency`/DAC 提前量维护可听绝对媒体时间；UI 用视频绝对时间计算 `av_offset_ms`。连续 3 次 2 秒轮询超过 80ms 才触发一次源重对齐，并设 10 秒冷却，避免瞬时抖动形成 IPC 风暴。主窗和最终效果窗的完整快照最多每 1 秒读取一次，PortAudio 状态最多每 2 秒读取一次，同类请求在途时不重复入队。
...
- 播放暂停时 output worker 暂停 callback 并让其补零；停止时清空环缓并在锁外停止、Join FFmpeg/解码/output 线程，避免按钮返回后仍有残余声音。停止后重新播放按当前播放位置重新预热；循环边界重建音频时间锚点并令旧候选失效。
- 实时 PortAudio 解码均使用 `-stream_loop -1`，禁止需要等待 EOF 的 `areverse`；实时淡出参数安全跳过，`areverse → afade → areverse` 仅用于有限输入离线链路。Scheduled 使用 `-re`，CatchUp 使用有界 `-readrate`，二者不得混写为同一个速度策略。
