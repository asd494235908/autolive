...
> 当前视频架构（2026-08-28，本文最高优先级）：最终效果窗口接入一个常驻受管 mpv 进程，使用 `gpu-next` 内置 libplacebo 执行 GPU83 自定义 shader 和 GPU 合成；当前最高开发与验收分辨率为 `1920×1080`，4K 延后。Windows 首选 D3D11/D3D11VA，Vulkan/WinVK 为第二路径。周期只通过受限 JSON IPC 原子更新一次完整参数快照，不重编码、不落盘、不切换视频源、不重建播放器。GPU83 失败时按会话单向降级到常驻 mpv 的 libavfilter CPU4，再失败保持 Original。PortAudio 可听 PTS 为普通声音启用时的主时钟，Rust 内部完成 `80ms` 内音画同步。
...
> 下一版本接线顺序（2026-08-28）：先完成同帧 shader、mpv PTS 调度和历史帧三类 GPU83 证据，再把现有 CPU4 命令编译器接入单一 mpv 会话状态机，最后把 `audio_cycle_output` 已有可听时间线接入 `media_av_sync` 与 mpv 持久 IPC。音频改动严格限制为时钟遥测和同步控制，不改 DSP、N/N+1、插话、固定话术及混音；4K 延后。
...
- 旧 MSE、fMP4 period、视频 `.partial/.ready`、逐周期 FFmpeg 视频 Worker、视频分块 IPC 和对应权限/测试已删除；视频生产入口只保留受管 mpv 与 Original。持久 IPC、受控 hook 和精确 83 项映射已落地；当前机器的 1080p D3D11/Vulkan/CPU4 实机矩阵、`vo-passes` 帧预算、同 PID 参数更新和丢帧时间线已通过。4K 延后；许可证材料、跨硬件、生产 CPU4 降级、真实同步和最终表面长稳尚未完成。仅 `20/83` 项完成 shader 语义验证，不能扩大宣称为完整 GPU83、所有硬件无卡顿或音画同步已经交付。
...
- GPU83 中依赖同帧中间纹理的效果继续使用 mpv user shader `SAVE/BIND`；真正依赖过去帧的独立画中画时间轴不能用同帧纹理冒充。固定随包 mpv 无法提供有界跨帧历史时，必须通过独立门禁升级到成熟 libmpv Render API/libplacebo 渲染边界，否则对应字段保持未接入。
...
- 受管 mpv 会话必须拥有持久 JSON IPC、唯一 request ID、有界读写队列、命令 deadline、stderr 尾缓冲、Job Object、取消和 Join；不得每条命令重开命名管道或无超时阻塞读取。
- 不再保留旧 MSE/period 的运行时代码作为审计对象；历史只由版本控制保存，不得从历史恢复生产调用。普通声音文件候选和视频导入时必要的一次性兼容引用不属于旧视频周期链。
...
- 管理 N/N+1 音视频临时文件、普通声音处理、插话和固定话术系统朗读状态；实时音频候选和 speech-to-speech 状态仅作历史兼容审计
...
- 支持以用户所选插话目录为根递归扫描其子目录、随机插话、主音轨 duck、插话独立音轨选择，以及普通声音处理中的单轨/多轨预设混合；普通声音与随机插话共用同一份 22 套、每套精确 35 字段的预设定义。插话以 `audio_selection_mode=fixed|random` 区分固定单轨与所选池随机，默认 `random`。`audio_fixed_preset_id` 可固定选择 `p01–p22`、默认 `p01`；随机模式的 `audio_preset_ids` 可多选或一键全选全部 22 项，以 `audio_mix_enabled + audio_mix_pick_min/audio_mix_pick_max` 表达随机单轨或每次 `1–4` 支路多轨合一。随机文件开始时先抽样一次，长文件按独立预设周期继续切换；`audio_variation_mode` 仅作兼容字段并统一写入 `periodic`。默认随机池为 `p01–p20`，`p21/p22` 只有用户显式选择后参与，其余默认 `false/1/2/periodic`。两条业务链共享预设事实源，但分别保存选择状态和运行快照
- 音频输入以当前源媒体音轨为准；普通声音处理写入 N/N+1 临时媒体文件，插话和固定话术走当前本地播放链路。实时 speech-to-speech 输入仅作后续版本预留
- 网络异常时保持本地播放
...
- 抽屉优化只调整信息结构和交互密度：高级声音抽屉保留多轨与预设、声音周期、状态和临时文件统计；预设区持续显示已选数量，并提供“选择默认 20 项 / 全选 22 项”快捷操作，底部只保留“重新生成本周期参数”。播放池完整编辑直接位于主页播放池 Card。“音高、变速与共振峰”“特征、信噪比与空间混合”位于主页声音列且不在抽屉重复。随机插话继续保存本地目录、间隔、音量、duck、选择模式和最多四支路设置；固定话术仍只使用 WebView2 本地系统声音。三个抽屉都不得新增实时话术幻化、speech-to-speech 或模型租约。
...
- FFmpeg/FFprobe：继续负责导入探测、普通声音候选、插话和必要的一次性兼容准备，不负责视频周期变换。
...
- 本地视频效果由 mpv 解码并通过 `gpu-next/libplacebo` 实时处理与呈现，正常周期不重新编码、不写视频候选文件；FFmpeg 只保留导入探测、必要的一次性视频兼容和声音侧职责，不承担视频周期变换、RTMP 推流、断线重连或推流子进程管理。
...
- 只有生产 mpv 对源素材真实不可播时，视频兼容准备才使用受管 FFmpeg 生成一次性 `playback_reference`；它与 `source_path` 分离，不按周期重建、不进入 MSE。候选批次任一失败、取消或被取代时清理本批无引用兼容缓存并保持旧池。
...
- 普通声音临时文件和必要的一次性视频兼容引用位于应用数据目录的受管 session 子目录。视频参数周期不产生临时文件；停止、换源、播放池编辑、窗口关闭和进程正常退出继续执行各自所有权范围内的清理。
...
### 6.8 MediaFileLeaseManager（声音候选与一次性兼容引用）
...
- 插话目录按流式递归目录迭代扫描，可播放文件最多 `1000` 个；目录和单文件规范化路径最多 `4096` UTF-8 字节，根目录与全部文件路径累计最多 `512 KiB`。任一上限超出时整体拒绝配置并返回稳定错误码，不保留部分目录结果；递归发现不改变既有格式白名单、安全校验或容量边界。随机插话抽屉从已保存的 `InterludeSnapshot.audio_count` 展示已识别数量与 `1000` 上限；草稿目录尚未提交时继续标明该数量属于已保存配置。
...
- 接收最多 `100` 个已全部探测成功的本地源媒体；视频使用只读源或必要的一次性兼容引用，纯音频保持原有声音链。
...
- 只读取用户选择的源媒体、独立兼容引用和应用受管临时文件，不上传媒体、不连接 RTMP。
...
- 一次性视频兼容缓存纳入同一受限缓存根和流式清理边界；播放池编辑、清空、窗口关闭和应用退出必须取消并 Join 兼容准备任务，保护仍由当前池引用的完成产物，只删除本批 `.partial`、失败产物和确认无引用的兼容缓存，不扩大递归删除目录。
- 声音 N/N+1 临时媒体写入稳定的应用自有 session 目录，并由 Tauri asset protocol 仅授权当前租约文件；可选视频一次性兼容引用和插话短期缓存使用各自独立目录、租约与清理边界。
...
本节区分目标契约与当前实现。当前源码已完成播放池多格式探测/编辑命令、FFprobe 探测、兼容播放引用，以及 mpv/libplacebo 持久会话 IPC、受控 shader、正式周期准备入口和 Phase 1 CPU4 四字段/实机/发布门禁；CPU4 尚未接入生产会话降级，最终表面、播放池 EOF 和真实音画闭环仍待实施。不应在界面中伪造已实现状态：
...
- `probe_local_video` / `probe_local_videos`：仅接受主窗口调用，统一媒体扩展名白名单校验后通过 FFprobe 读取实际容器和流元数据；返回规范化源路径、媒体类型、视频/音频流、时长、采样率、声道、`playback_reference` 和 `compatibility_mode`。`probe_local_videos` 仅在全部候选探测及必要兼容准备成功后原子替换播放池；取消、超限或任一项失败都保持旧池并显示错误，不保留部分成功项。成功后首项保持 `Ready`，不会自动打开窗口或播放。
- 现有探测/编辑 IPC 已收口为统一媒体语义；纯音频格式进入同一原子批处理并保持直通。视频优先由 mpv 直接读取源文件，只有真实不可播的条目才在候选批次提交前完成一次性兼容准备；任何单项失败或取消都不更新 `source_media_pool`，也不把 `.partial` 或未校验产物暴露给 WebView。
...
- `prepare_realtime_video_plan` 已使用有限允许列表启动/复用受管 mpv，并以一条 `set_property glsl-shader-opts` 原子提交已验证参数快照。底层使用一条持久命名管道、唯一 `request_id`、有界 pending/响应和命令 deadline；旧视频分块 IPC 已删除。
...
- 当前播放池实现一次多选 `1～100` 个白名单媒体文件后按返回顺序逐项探测，全部成功才原子替换旧池。视频优先由生产 mpv 直接读取 `source_path`，只在不可播时使用一次性兼容引用。新池首项保持 `Ready`，用户点击“播放”才创建/聚焦单实例最终窗口并启动受管 mpv；项结束后复用同一会话按序换源。
...
- 原始视频只读保留；实时视频效果不落盘。仅在 mpv 真实不可播时生成的一次性视频兼容缓存、插话短期音频缓存和用户显式导出使用各自受限目录并可清理。
...
- 处理失败回退原片，原始文件不被覆盖
- 验证 GPU83、CPU4 和 Original 三种互斥会话状态；正常周期只发送一次参数快照，不创建文件、不切源、不重启 mpv。检查最终画面没有 CSS 叠加，在正常观看下确认完整 83 项组合低感知，再用明显可见手工对照档逐项确认像素真实变化。依次注入 GPU83 不可用/超时和 CPU4 失败，确认按会话单向降级、候选释放或进入有界重试且继续播放 Original，无黑帧或双声；导入兼容准备失败、取消和旧池保留另行验收。
- 弹窗关闭和重新打开
- 断网继续播放
...
- 在 Windows release 验证 mpv 优先直接播放长 GOP H.264、HEVC 和其他白名单素材，只对真实不可播源生成一次性兼容引用；覆盖损坏视频、取消及 `.partial`/无引用缓存清理。逐一验证 `.mp3/.wav/.m4a/.aac/.ogg/.flac` 纯音频直通、带 `attached_pic` 音频和视频 → 音频 → 视频连续两轮混排；纯音频期间普通声音、插话、固定话术及播放控制保持可用。
