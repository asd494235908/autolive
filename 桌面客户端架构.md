...
桌面 UI 不展示独立“运行资源”卡片，不向用户提供 FFmpeg/FFprobe 安装、取消、选择目录或清理资源按钮。FFmpeg/FFprobe 继续作为安装包和内部媒体执行依赖，缺失、探测失败或处理失败通过导入/处理错误反馈。
...
- 维护普通声音处理的 FFmpeg 参数配置、处理缓存状态和“下一轮切换”状态；Web Audio 只用于只读波形/频谱观测，不执行声音效果
- 普通声音处理的用户入口提供 21 项可映射 FFmpeg 声音参数：增益/响度、三段 EQ/Q 值、音高、变速、淡入淡出、轻混响、降噪、相位、颤音、环境噪声比例/电平、采样率和码率。用户修改后点击应用，处理完成后显示 `ready`；环境声素材混合、干湿比、压缩及高级 DSP 保持 `unavailable`
- 研究分析模式完整配置 MFCC 维度/偏移、SNR 浮动、共振峰偏移、滤波 Q 值、相位扰动、颤音频率/深度、频谱扰动/盲区和音色库
- 研究分析模式只读展示当前 SNR、当前共振峰、可用音色数量及其他实时波形/频谱诊断；只读值不得作为处理参数提交
- 提供响度稳定、实时波形、频谱和多频段动态联动数据
- 将用户配置、只读诊断、资源统计和运行状态分成不同字段；`SNR 浮动`、`当前 SNR`、`共振峰偏移`、`当前共振峰`、`音色库`、`可用音色数量`不得混用
...
- 本节不扩展实时音频幻化参数，实时幻化继续使用独立 Worker 和第 6 节既有参数契约
...
- 展示分辨率、帧率、时长、编码格式，以及已计算的源文件/当前产物完整 SHA-256 和阶段哈希；输入或产物实际为 MP4 且进入对应哈希链路时额外展示 MP4 专用哈希
...
当前固定话术采用独立的本地系统朗读链路，和实时 speech-to-speech、普通声音处理互相独立：主窗口通过 `autolive-playback-ui-v1` 的 BroadcastChannel 向同一实例的 `final-effect` 窗口发送版本化 `fixed-speech-command`，最终效果窗口只选择 `SpeechSynthesisVoice.localService === true` 的本地声音，调用 WebView2 的 `speechSynthesis` 朗读 1～500 个 Unicode 字符。朗读期间视频继续播放，但视频和候选音轨临时静音、插话暂停；`start/end/error/cancel`、切换源视频、停止和窗口卸载都通过操作 ID 幂等清理并恢复用户原来的静音/音量状态。固定话术不识别人声、不分离背景、不生成 WAV/MP4、不调用 Rust 固定话术 IPC、不依赖 Python/Worker/ASR/TTS 模型；没有本地声音或 WebView2 能力时 fail-closed，保持原音轨。正式 EXE 必须通过真实 Windows WebView2 系统朗读烟测后才能发布。
...
- 媒体缓存索引和清理状态
- 当前源视频、播放轮次和最后预览状态
- 研究分析配置、算法版本、随机种子、源文件/当前产物完整 SHA-256、条件 MP4 专用哈希、阶段哈希和研究报告
...
- `probe_local_video`：仅接受主窗口调用，扩展名白名单校验后通过 FFprobe 读取实际容器和流元数据；返回规范化源路径、视频/音频流、时长、采样率和声道。探测成功后尝试原声直放；当前不生成自动兼容预览缓存。
...
- `set_audio_processing_profile`：仅接受主窗口调用，校验普通声音处理和研究参数的版本、角色、单位和范围；21 项可映射声音参数由后续 `start_media_processing` 交给 FFmpeg，处理缓存确认后才返回 `ready`。播放速度不参与周期调度；Web Audio 只读诊断，不执行声音效果。未接入的环境声素材混合、干湿比、压缩和高级分析字段保持 `unavailable`，只读诊断不得作为处理参数提交。
...
- 目标真实预览：选择一个白名单文件后调用 `probe_local_video`，只有实际探测成功项才可播放；由 `convertFileSrc(canonical_path)` 生成 asset URL，能否直放由目标 WebView/编码器决定，失败进入导入/播放错误链路。最终效果窗口由 `open_final_effect_window` 单实例创建并循环加载当前视频引用。主窗口和最终效果窗口通过 `get_snapshot` 轮询同步播放轮次、当前/待切换视频、音轨状态和处理开关；视频处理只在循环边界切换，不因后台处理或音频候选重新弹窗或创建第二个视频版本。
...
客户端不直接访问 PostgreSQL，不直接读取后台密钥配置，也不把本地视频内容上传到 Go API。客户端先在本地探测源素材；处理或研究阶段按契约计算输入/产物完整 SHA-256。当前播放无需创建媒体任务，Go 只承载登录、激活、号池/直连租约、账号测试、调用摘要和状态摘要。实时音频幻化的 speech-to-speech 输入、输出、模型状态和换轨均在桌面端本地或受控 Worker 执行。内容指纹对比只属于研究报告，不参与实时播放决策。
...
- 清理过期的临时租约和失效会话。
- 可恢复独立保存的研究报告和参数配置；实时播放、实时话术和固定话术的进程内状态不跨重启恢复。
...
- 第 6.7 节全部普通声音处理/研究字段的保存、重置、只读诊断隔离和 `disabled/configured/runtime/unavailable` 状态转换
- 视频多频段、帧率、像素、动态裁剪、空间偏移和挂件参数保存与重置
- 整个源文件/当前产物的 SHA-256、条件 MP4 专用哈希、阶段哈希、算法版本、随机种子和研究报告一致性
