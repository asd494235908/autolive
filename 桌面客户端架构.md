...
> 视频降级状态 DTO 闭环（2026-08-30，代码及聚焦测试完成、待用户真实媒体验收）：视频后端从活动 GPU/CPU4 会话进入 `Configured/Probing` 降级状态时，必须在清除进程事实的同一 runtime actor 边界重置 AV 同步控制器、音频时钟身份、`av_sync_drift_ms`、`audible_audio_pts_ms` 与 `audio_epoch`。三项对外音频时钟字段必须同时存在，且只允许属于具有真实 `process_id` 的 `Active` 状态；无活动进程的状态不得残留上一后端的音频时钟。该约束避免 React 严格 DTO 校验拒绝降级响应并反复释放、重建视频候选，不改变 PortAudio DSP、声音候选、GPU83/CPU4 参数能力或单向降级顺序。
...
> 虚拟摄像头方案（2026-08-30，正式需求·待实施）：唯一后端采用 AkVirtualCamera，首版固定 `GpAutoLive Camera`、Windows 10/11 DirectShow 和 YUY2 `1280×720@30fps`。最终效果 HWND 经 WGC/D3D11 做 GPU 捕获、缩放和转换，再通过一次有界 staging readback 交给独立 GPL sidecar；必须显示 `zero_copy=false`。Windows 11 MF、NV12、1080p 和共享纹理零拷贝均未通过门禁，不属于当前能力。

> 麦克风插话范围（2026-08-30，正式需求·待实施）：现有随机文件插话的用户可见入口统一改名为“插话文件”；新增用户显式开启的本地麦克风插话。麦克风链仅允许使用 PortAudio 与 SpeexDSP 完成回声消除、降噪、自动增益和非语义 VAD，并在说话期间把主媒体及其他话术源的最终数字增益门控到 `0`，不做 ASR、LLM、TTS、speech-to-speech、录音落盘或上传。详细边界、模块和验收以 [`麦克风插话与插话文件入口实施方案`](./docs/superpowers/plans/2026-08-30-麦克风插话与插话文件入口实施方案.md) 为准；在实现和真实硬件验证前不得标记为可用。

> ZLMediaKit RTMP/RTMPS 直推范围（2026-08-30，正式需求·待实施）：用户输入完整发布地址，默认同时输出画面和声音，也可只选其一。桌面端直接读取当前活动源和最终 PCM 总线，通过独立受管 FFmpeg 发布链复用 libplacebo/GPU83 静态快照与硬件 H.264 编码器探测；不捕获桌面、最终效果窗口或 HWND，不经过 Go，也不接入 OBS、平台账号和多平台分发。详细边界以 [`ZLMediaKit RTMP GPU 直推实施方案`](./docs/superpowers/plans/2026-08-30-ZLMediaKit-RTMP-GPU直推实施方案.md) 为准；实现和真实 ZLMediaKit 门禁完成前不得标记为可用。
...
- 管理 N/N+1 音视频临时文件、普通声音处理、插话文件和固定话术系统朗读状态；后续按专项方案管理本地麦克风插话状态。实时音频候选和 speech-to-speech 状态仅作历史兼容审计
...
- 当前已实现的音频输入以源媒体音轨和插话文件为准；普通声音处理写入 N/N+1 临时媒体文件，插话文件和固定话术走当前本地播放链路。麦克风输入按专项方案待实施；实时 speech-to-speech 输入仅作后续版本预留
- 网络异常时保持本地播放
...
| ZLMediaKit RTMP/RTMPS 直推 | 用户输入完整地址，默认音画双轨且可只选一轨；直接源处理与最终 PCM 分流，不捕获窗口、不经过 Go | 正式需求·待实施 |
...
- 参考图同时定义视觉结构与正式媒体参数需求。当前已实现 UI 不出现实时话术幻化、speech-to-speech、旧报告/变体 MP4 产物、内容指纹、OBS 或平台推流；ZLMediaKit RTMP/RTMPS 卡片只在专项实施时加入，并在真实发布门禁完成前明确显示“待实施/未接入”。像素扰动、帧率扰动和 `65Hz–20000Hz` 视觉频段可按能力状态展示，但未映射时不得宣传为已生效，更不得宣传检测规避。
- 深色背景必须从应用启动壳到 React 首次可交互帧保持一致，避免白色启动闪烁。所有颜色、间距、字号、圆角和控件状态通过现有 Design Token、Ant Design 公开 Props 和外层布局实现，不覆盖 `.ant-*` 内部样式。

#### 1.2.1 三个声音抽屉统一规范

- “高级声音”抽屉的桌面目标宽度为 `760px`，“插话文件”“麦克风插话”和“固定话术”抽屉的桌面目标宽度均为 `560px`；四者最终宽度都不得超过 `100vw`，窄窗口下收敛到当前可用视口宽度，不产生抽屉外的横向滚动。麦克风插话尚未实施时不得显示可点击的伪入口。
...
flowchart TB
    ...
    Window --> WGC["WGC / D3D11 Capture（待实施）"]
...
### 6.14.1 MicrophoneInterlude（正式需求·待实施）
...
- `MicrophoneSpeechDsp` 在受管工作线程中使用成熟 SpeexDSP，处理顺序固定为 AEC → 降噪 → 自动增益 → VAD。AEC 参考信号取最终主输出在麦克风叠加前的近端播放参考；输入输出设备不共享稳定时钟或参考不可用时，明确降级为 NS/AGC/VAD 并显示“回声消除不可用”，不得伪造运行成功。
...
- 声音在最终 PCM 完成主轨增益、普通声音处理、交叉淡化、插话 duck/叠加和限幅后，经有界 `RtmpAudioSink` 分流为 `48kHz` 双声道 AAC；不能阻塞 PortAudio 实时回调，溢出只按明确策略计数和报错。固定话术在 PCM 未迁入 Rust 总线前必须显示“不进入当前推流音轨”。
...
- 是虚拟摄像头 session 的唯一所有者，独占 generation、WGC frame pool、D3D11 device/context、三槽 staging、容量 1 latest-wins 队列、独立 GPL sidecar、取消、deadline、Job Object 和 Join；`FinalPreviewController` 只提供受管最终效果 HWND 与播放事实，不拥有其生命周期。
- 数据链固定为 `mpv/libplacebo 最终效果 HWND → WGC ID3D11Texture2D → D3D11 Video Processor GPU 缩放/BGRA→YUY2 → 一次 staging readback → AkVirtualCamera direct-mode mmap → DirectShow`。不得启动第二 mpv、二次解码、CPU 重做 GPU83、GDI 截图或逐帧编码。
...
虚拟摄像头目标 IPC 为 `get_virtual_camera_status`、`install_or_repair_virtual_camera`、`start_virtual_camera_output`、`stop_virtual_camera_output`。四者只接受 `main` 窗口，安装/修复使用签名固定 helper 与固定 verb，不允许前端传入任意路径、命令、CLSID、设备 ID 或 shell 参数；实现前不注册、不返回伪成功。`final-effect` 只通过内部 Rust 窗口生命周期提供 HWND，不获得安装或控制权限。
...
- 2026-08-30：新增 `MicrophoneInterlude` 待实施架构和专项方案；复用 PortAudio 全双工输入并优先采用 SpeexDSP AEC/NS/AGC/VAD，麦克风说话期间由唯一最终混音门控把其他声音数字静音到 `0`。同时将现有随机文件插话的用户可见名称统一为“插话文件”；该变更尚未实施，不扩大到 ASR、LLM、TTS、speech-to-speech、录音或上传。
