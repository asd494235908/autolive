...
> C# 抖音登录诊断边界（2026-09-10，实施中）：Python 仅输出固定白名单登录诊断事件，Windows Host 严格校验后负责本机有界日志，WPF 只投影日志路径与保存状态；禁止直接转存上游 stdout/stderr 或异常正文。不新增账号持久化、验证码流程、模型或 Go 调用。见 [实施记录](./desktop-csharp-windows/docs/2026-09-10-抖音登录脱敏日志实施记录.md)。

> C# 抖音会话边界补充（2026-09-10，实施中）：Windows Host 继续作为登录有效性和房间生命周期的唯一所有者，WPF 分别投影两者；普通房间失败不注销，同房间重复连接不重新扫码。认证失效、进程退出、本地通信异常分别保留脱敏原因，状态未知不得继续发送；重启仍需扫码。不增加自动重连、凭据持久化或其他端调用，见 [修复记录](./desktop-csharp-windows/docs/2026-09-10-抖音登录与房间连接分离修复.md)。

> C# 抖音手动发送增量（2026-09-10，代码与定向验证完成）：WPF 弹幕窗口仅输入/投影，MainWindow 持有单次任务并由 Host 执行发送；手动和自动链路共用 canonical 单条发送、限频与互斥，不新增 sidecar、队列或凭据存储。只有显式点击才提交，自动回应默认关闭；关闭查看窗口后发送仍由主窗口监督，房间/应用退出不得把旧消息发到新房间。平台接收结果与列表中的本人回显分别表达，真实发送仍待验收。

> C# 媒体同步增量（2026-09-09）：主音轨读取帧计数 → PortAudio 可听时钟（扣除 DAC 延迟、使用当前音轨倍速）→ MainWindow 播放串行边界 → mpv 调速/严重漂移 seek。首轨与 N+1 复用 FinalPcmBus 容量背压，避免 `-re` 限速造成倍速欠载；视频换源保帧与暂停加载共用同一 mpv，RTMP 保存/恢复配置但仍重建单源发布连接。实现与验收边界见 [播放同步与换源修复](./desktop-csharp-windows/docs/2026-09-09-播放同步与换源修复.md)。

> C# 虚拟摄像头覆盖口径（2026-09-09，实施中）：当前客户端为 C#/WPF。输出跟随源视频分辨率，YUY2 宽度向上偶数对齐，边长 ≤4096、总像素 ≤4096×2160、30fps。GPU 使用 D3D11 shader，捕获仍只限最终效果 HWND。动态 v2 帧头保留 52 字节，v1 仅兼容旧 720p。开发安装仅向当前用户开放专用设备格式子键的尺寸配置；显式开发包校验全组件哈希后允许未签名，生产包仍强签名。下文旧固定 720p/PnP/测试强签名要求不再约束此 C# 增量，详见 [实施记录](./desktop-csharp-windows/docs/2026-09-09-虚拟摄像头源分辨率与测试组件实施记录.md)。

> C# 抖音登录生命周期修复（2026-09-09，代码与定向验证完成）：Host 区分普通直播间断开与完整停止；前者保留受管 sidecar 和内存认证，只终止本轮监听/任务，后者仍执行 logout、shutdown 和资源回收。重连复用原进程但新建房间 generation，不保留旧任务，同登录的限频与风控阻断不重置，不增加凭据落盘或自动重连框架。真实重连/单次发送结果以专项实施记录为准。

> C# 抖音显示链路覆盖（2026-09-09，代码已接入，真实扫码与弹幕显示已验证）：使用 C# 专属 canonical Python sidecar 持续接收 `WebcastChatMessage`；Windows 宿主保留本轮最近 500 条显示 DTO，WPF 单实例窗口投影昵称/正文/时间，自动回复由原 Core 队列独立控制。此需求允许正文进入 C# 本地内存显示，不进入 Go、普通日志或持久化；复用既有会话校验、取消和退出所有权，旧 Rust 探针不改。详见 [实施方案与验收记录](./desktop-csharp-windows/docs/2026-09-09-抖音实时弹幕窗口实施方案.md)。

> C# 实现覆盖（2026-09-08）：当前 C# 视频切换以 `MainWindow.VideoTransitions` 统一编排旧帧截图、同级原生 HWND 保帧及确认后撤层；Media/Windows 负责受限截图 IPC、加载事件代次与真实输出确认。保持单 mpv、独立音频及 RTMP 链路。失败保帧与资源释放规则详见 [视频切换保帧修复](./desktop-csharp-windows/docs/2026-09-08-视频切换保帧修复.md)。

> C# 安装器覆盖（2026-09-07）：当前 C# WPF/SAPI 桌面端的跨机安装入口是独立 NSIS，不复用依赖 `.NET 10 + PowerShell 7` 的 `GpAutoLive.Setup.exe` 维护壳，也不携带未使用的 WebView2。NSIS 离线包含 `.NET 10 Windows Desktop Runtime x64` 和完整已核验应用目录，安装到 `%ProgramFiles%\GpAutoLive`；运行中拒绝升级/卸载，同卷 staging 失败时恢复旧 `app`，卸载保留 `%LocalAppData%` 用户配置、Credential Manager 凭据和用户媒体。签名、法律审核、干净 Windows 10/11 与真实业务冒烟仍是正式发布门禁。

> C# 安装包控制面隔离（2026-09-07）：NSIS 在应用目录写入固定 `GpAutoLive.control-plane-profile`。`cloud-test-v1` 默认使用唯一固定测试云地址和 `GpAutoLive.CSharp.Windows.Test/` 凭据前缀，`offline` 可强制禁网；`production-v1` 不接受 `test/development` 遗留配置或固定测试 HTTP，只接受显式 HTTPS。原始源码开发输出无包标记，继续使用 launch profile；安装器不写系统或用户全局环境变量。
