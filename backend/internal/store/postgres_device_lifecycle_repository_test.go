...
func TestPostgresRepositoryUnbindDeviceCommitsLifecycle(t *testing.T) {
	...
	mock.ExpectExec(regexp.QuoteMeta("UPDATE model_leases SET status = $2, released_at = $3")).WithArgs("dev_1", controlplane.ModelLeaseStatusReleased, now, controlplane.ModelLeaseStatusActive).WillReturnResult(sqlmock.NewResult(1, 1))
	mock.ExpectExec(regexp.QuoteMeta("UPDATE auth_sessions SET revoked_at = CURRENT_TIMESTAMP")).WithArgs("dev_1").WillReturnResult(sqlmock.NewResult(1, 1))
	mock.ExpectQuery(regexp.QuoteMeta("SELECT activation_code_id FROM activation_device_bindings")).WithArgs("dev_1", controlplane.ProductAutoLive).WillReturnRows(sqlmock.NewRows([]string{"activation_code_id"}).AddRow("ac_1"))
	mock.ExpectExec(regexp.QuoteMeta("DELETE FROM activation_device_bindings WHERE device_id = $1")).WithArgs("dev_1").WillReturnResult(sqlmock.NewResult(1, 1))
	mock.ExpectExec(regexp.QuoteMeta("UPDATE activation_codes SET bound_devices = GREATEST(bound_devices - 1, 0)")).WithArgs("ac_1", controlplane.ActivationCodeStatusUsed, now, controlplane.ActivationCodeStatusActive, controlplane.ProductAutoLive).WillReturnResult(sqlmock.NewResult(1, 1))
