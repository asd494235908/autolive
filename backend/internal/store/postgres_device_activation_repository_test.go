...
func TestPostgresRepositoryActivateDeviceWithSessionBindingCommitsTogether(t *testing.T) {
	...
	mock.ExpectQuery(regexp.QuoteMeta("SELECT user_id, device_id FROM auth_sessions")).WithArgs("access-hash").WillReturnRows(sqlmock.NewRows([]string{"user_id", "device_id"}).AddRow("usr_1", nil))
	...
	mock.ExpectQuery(regexp.QuoteMeta("SELECT id, user_id, device_name, platform, client_version, status, last_heartbeat_at FROM devices")).WithArgs("dev_1").WillReturnRows(sqlmock.NewRows([]string{"id", "user_id", "device_name", "platform", "client_version", "status", "last_heartbeat_at"}))
	mock.ExpectQuery(regexp.QuoteMeta("SELECT id, status, expires_at, max_devices, bound_devices FROM activation_codes")).WithArgs("digest-1").WillReturnRows(sqlmock.NewRows([]string{"id", "status", "expires_at", "max_devices", "bound_devices"}).AddRow("ac_1", controlplane.ActivationCodeStatusActive, expiresAt, 2, 0))
	mock.ExpectExec(regexp.QuoteMeta("INSERT INTO devices (id, user_id, device_key, device_name, platform, client_version, status, last_heartbeat_at)")).WithArgs("dev_1", "usr_1", "state-device/dev_1", "Demo", "windows", "1.0.0", controlplane.DeviceStatusActive, now).WillReturnResult(sqlmock.NewResult(1, 1))
	mock.ExpectExec(regexp.QuoteMeta("UPDATE activation_codes SET status = $2, bound_devices = $3, used_at = COALESCE(used_at, $4), used_by_user_id = COALESCE(used_by_user_id, $5), used_by_device_id = COALESCE(used_by_device_id, $6)")).WithArgs("ac_1", controlplane.ActivationCodeStatusActive, 1, now, "usr_1", "dev_1").WillReturnResult(sqlmock.NewResult(1, 1))
	mock.ExpectExec(regexp.QuoteMeta("UPDATE auth_sessions SET device_id = $2, device_bound_at = CURRENT_TIMESTAMP")).WithArgs("access-hash", "dev_1").WillReturnResult(sqlmock.NewResult(1, 1))
	...
	device, err := repository.ActivateDeviceWithSessionBinding(context.Background(), DeviceActivationRecord{
		...
		Device: controlplane.DeviceRegistration{DeviceID: "dev_1", DeviceName: "Demo", Platform: "windows", AppVersion: "1.0.0"},
...
func TestPostgresRepositoryActivateDeviceAuditFailureRollsBackBusinessTransaction(t *testing.T) {
	...
	mock.ExpectQuery(regexp.QuoteMeta("SELECT id, username, role, status, created_at FROM users")).WithArgs("usr_1").WillReturnRows(sqlmock.NewRows([]string{"id", "username", "role", "status", "created_at"}).AddRow("usr_1", "alice", controlplane.RoleUser, controlplane.UserStatusActive, now))
	mock.ExpectQuery(regexp.QuoteMeta("SELECT id, user_id, device_name, platform, client_version, status, last_heartbeat_at FROM devices")).WithArgs("dev_1").WillReturnRows(sqlmock.NewRows([]string{"id", "user_id", "device_name", "platform", "client_version", "status", "last_heartbeat_at"}))
	mock.ExpectQuery(regexp.QuoteMeta("SELECT id, status, expires_at, max_devices, bound_devices FROM activation_codes")).WithArgs("digest-1").WillReturnRows(sqlmock.NewRows([]string{"id", "status", "expires_at", "max_devices", "bound_devices"}).AddRow("ac_1", controlplane.ActivationCodeStatusActive, expiresAt, 1, 0))
	mock.ExpectExec(regexp.QuoteMeta("INSERT INTO devices (id, user_id, device_key, device_name, platform, client_version, status, last_heartbeat_at)")).WithArgs("dev_1", "usr_1", "state-device/dev_1", "Demo", "windows", "1.0.0", controlplane.DeviceStatusActive, now).WillReturnResult(sqlmock.NewResult(1, 1))
	mock.ExpectExec(regexp.QuoteMeta("UPDATE activation_codes SET status = $2, bound_devices = $3, used_at = COALESCE(used_at, $4), used_by_user_id = COALESCE(used_by_user_id, $5), used_by_device_id = COALESCE(used_by_device_id, $6)")).WithArgs("ac_1", controlplane.ActivationCodeStatusUsed, 1, now, "usr_1", "dev_1").WillReturnResult(sqlmock.NewResult(1, 1))
	mock.ExpectExec(regexp.QuoteMeta("UPDATE auth_sessions SET device_id = $2, device_bound_at = CURRENT_TIMESTAMP")).WithArgs("access-hash", "dev_1").WillReturnResult(sqlmock.NewResult(1, 1))
	mock.ExpectExec(regexp.QuoteMeta("INSERT INTO audit_outbox (")).WithArgs(sqlmock.AnyArg(), "audit-request:req-fail", sqlmock.AnyArg(), now).WillReturnError(errors.New("outbox unavailable"))
	mock.ExpectRollback()
