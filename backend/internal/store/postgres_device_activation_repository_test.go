...
func TestPostgresRepositoryActivateDeviceWithSessionBindingCommitsTogether(t *testing.T) {
	...
	mock.ExpectQuery(regexp.QuoteMeta("SELECT id, status, expires_at FROM activation_codes")).WithArgs("digest-1").WillReturnRows(sqlmock.NewRows([]string{"id", "status", "expires_at"}).AddRow("ac_1", controlplane.ActivationCodeStatusActive, expiresAt))
	...
	mock.ExpectExec(regexp.QuoteMeta("UPDATE activation_codes SET status = $2, used_at = $3, used_by_user_id = $4, used_by_device_id = $5")).WithArgs("ac_1", controlplane.ActivationCodeStatusUsed, now, "usr_1", "dev_1").WillReturnResult(sqlmock.NewResult(1, 1))
