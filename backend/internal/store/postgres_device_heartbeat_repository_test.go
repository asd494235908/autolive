...
func TestPostgresRepositoryRecordHeartbeatWithSessionBindingCommitsTogether(t *testing.T) {
	...
	mock.ExpectExec(regexp.QuoteMeta("UPDATE devices SET disk_free_bytes = $2")).WithArgs("dev_1", int64(1024), int64(4096), int64(2048), 8, "windows", "11", "kernel", "demo.mp4", "playing", now).WillReturnResult(sqlmock.NewResult(1, 1))
	...
	result, err := repository.RecordHeartbeatWithSessionBinding(context.Background(), DeviceHeartbeatRecord{
		...
		Input: controlplane.HeartbeatInput{DeviceID: "dev_1", SentAt: now, Status: controlplane.HeartbeatStatus{DiskFreeBytes: 1024, MemoryTotalBytes: 4096, MemoryAvailableBytes: 2048, CPULogicalCores: 8, OSName: "windows", OSVersion: "11", KernelVersion: "kernel", CurrentMediaName: "demo.mp4", PlaybackState: "playing"}},
