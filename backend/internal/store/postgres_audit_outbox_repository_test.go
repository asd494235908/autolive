...
func TestPostgresRepositoryRecordAuditWithOutboxDeliversOnce(t *testing.T) {
	...
	mock.ExpectCommit()
	mock.ExpectBegin()
	mock.ExpectQuery(regexp.QuoteMeta("SELECT id, payload\n\t\tFROM audit_outbox")).WithArgs(now, now.Add(-auditOutboxProcessingGrace), 1).
		WillReturnRows(sqlmock.NewRows([]string{"id", "payload"}).AddRow("audit_outbox_1", []byte(`{"Action":"model.lease.release","TargetType":"model_lease","RequestID":"req-1","Outcome":"success","StatusCode":200}`)))
	...
	mock.ExpectCommit()
	mock.ExpectBegin()
	mock.ExpectExec(regexp.QuoteMeta("INSERT INTO audit_logs (")).WithArgs("audit_1", "", "", "model.lease.release", "model_lease", "", "req-1", "success", 200, "", now).WillReturnResult(sqlmock.NewResult(1, 1))
...
func TestPostgresRepositoryRecordAuditWithOutboxMarksDeliveryFailureRetryable(t *testing.T) {
	...
	mock.ExpectBegin()
	mock.ExpectExec(regexp.QuoteMeta("INSERT INTO audit_outbox (")).WithArgs(sqlmock.AnyArg(), "audit-request:req-2", sqlmock.AnyArg(), now).WillReturnResult(sqlmock.NewResult(1, 1))
	mock.ExpectCommit()
	mock.ExpectBegin()
	mock.ExpectQuery(regexp.QuoteMeta("SELECT id, payload\n\t\tFROM audit_outbox")).WithArgs(now, now.Add(-auditOutboxProcessingGrace), 1).
		WillReturnRows(sqlmock.NewRows([]string{"id", "payload"}).AddRow("audit_outbox_2", []byte(`{"Action":"test","TargetType":"test","RequestID":"req-2","Outcome":"success","StatusCode":200}`)))
	...
	mock.ExpectCommit()
	mock.ExpectBegin()
	mock.ExpectExec(regexp.QuoteMeta("INSERT INTO audit_logs (")).WithArgs("audit_2", "", "", "test", "test", "", "req-2", "success", 200, "", now).WillReturnError(errors.New("audit table unavailable"))
	mock.ExpectRollback()
	mock.ExpectBegin()
