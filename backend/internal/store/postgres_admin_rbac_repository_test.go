...
func TestPostgresRepositoryGetAdminAuthorizationUsesScopedBindingsAndGlobalSuperAdmin(t *testing.T) {
	...
	if !slices.Equal(authorization.Permissions, normalizedCatalogPermissions()) {
...
func TestPostgresRepositoryListAdminPermissionsUsesBoundedQuery(t *testing.T) {
	database, mock, err := sqlmock.New()
	if err != nil {
		t.Fatalf("sqlmock.New() error = %v", err)
	}
	...
	mock.ExpectQuery(regexp.QuoteMeta("SELECT code FROM admin_permissions ORDER BY code LIMIT $1")).
		WithArgs(adminRBACListLimit).
	...
	if !slices.Equal(permissions, []controlplane.PermissionCode{"devices.read", "users.read"}) {
...
func TestPostgresRepositoryAdminRBACListAdminRolesReservesSlotForGlobalSuperAdmin(t *testing.T) {
	database, mock, err := sqlmock.New()
	if err != nil {
		t.Fatalf("sqlmock.New() error = %v", err)
	}
	...
	for i := 0; i < 199; i++ {
		...
		rows.AddRow(code, nil, name, false, "devices.read")
		rows.AddRow(code, nil, name, false, "users.read")
	}
	...
	mock.ExpectQuery(regexp.QuoteMeta(`
		...
		ORDER BY r.code ASC, r.product ASC, rp.permission_code ASC
	`)).
		WithArgs("", adminRBACScopedRoleLimit).
		WillReturnRows(rows)

	roles, err := repository.ListAdminRoles(context.Background(), "")
	if err != nil {
		t.Fatalf("ListAdminRoles() error = %v", err)
	}
	if len(roles) != adminRBACListLimit {
		t.Fatalf("len(roles) = %d, want %d", len(roles), adminRBACListLimit)
	}
	...
	if roles[198].Code != "role_198" || !slices.Equal(roles[198].Permissions, []controlplane.PermissionCode{"devices.read", "users.read"}) {
		t.Fatalf("roles[198] = %+v", roles[198])
	}
	if roles[199].Code != controlplane.BuiltinAdminRoleSuperAdmin || !slices.Equal(roles[199].Permissions, []controlplane.PermissionCode{"users.manage"}) {
		t.Fatalf("roles[199] = %+v", roles[199])
	}
	if err := mock.ExpectationsWereMet(); err != nil {
		t.Fatalf("sql expectations: %v", err)
	}
...
func TestPostgresRepositoryAdminRBACListAdminRolesForProductAllowsTwoHundredOrdinaryRolesPlusGlobalSuperAdmin(t *testing.T) {
	database, mock, err := sqlmock.New()
	if err != nil {
		t.Fatalf("sqlmock.New() error = %v", err)
	}
	...
	if len(roles) != adminRBACListLimit+1 {
		t.Fatalf("len(roles) = %d, want %d", len(roles), adminRBACListLimit+1)
	}
	if roles[199].Code != "role_199" {
		t.Fatalf("roles[199] = %+v, want role_199", roles[199])
	}
	if roles[200].Code != controlplane.BuiltinAdminRoleSuperAdmin {
		t.Fatalf("roles[200] = %+v, want global super_admin", roles[200])
	}
	if err := mock.ExpectationsWereMet(); err != nil {
		t.Fatalf("sql expectations: %v", err)
	}
...
func TestPostgresRepositoryReplaceUserAdminRolesReplayAddsCompatibilityLocalSuperAdmin(t *testing.T) {
	database, mock, err := sqlmock.New()
	if err != nil {
		t.Fatalf("sqlmock.New() error = %v", err)
	}
	...
	mock.ExpectQuery(regexp.QuoteMeta("INSERT INTO idempotency_records (scope, idempotency_key, fingerprint, resource_id, created_at)")).
		WithArgs("control-plane-state", "replace-user-admin-roles:usr_local_admin:replay", "fp-local-replay", "usr_local_admin", now).
	...
	mock.ExpectQuery(regexp.QuoteMeta("SELECT fingerprint, resource_id FROM idempotency_records WHERE scope = $1 AND idempotency_key = $2 FOR UPDATE")).
		WithArgs("control-plane-state", "replace-user-admin-roles:usr_local_admin:replay").
		WillReturnRows(sqlmock.NewRows([]string{"fingerprint", "resource_id"}).AddRow("fp-local-replay", "usr_local_admin"))
	mock.ExpectQuery(regexp.QuoteMeta(`
...
func TestPostgresRepositoryListUserAdminRolesAddsCompatibilityLocalSuperAdminAndUsesLimit(t *testing.T) {
	database, mock, err := sqlmock.New()
	if err != nil {
		t.Fatalf("sqlmock.New() error = %v", err)
	}
	...
	mock.ExpectQuery(regexp.QuoteMeta(`
		...
		LIMIT $3
	`)).
		WithArgs("usr_local_admin", controlplane.ProductAutoLive, adminRBACListLimit).
		WillReturnRows(sqlmock.NewRows([]string{"role_code", "product"}).
			AddRow("ops_autolive", controlplane.ProductAutoLive))

	assignments, err := repository.ListUserAdminRoles(context.Background(), "usr_local_admin", controlplane.ProductAutoLive)
	if err != nil {
...
func TestPostgresRepositoryReplaceUserAdminRolesIsIdempotentAndProtectsLastSuperAdmin(t *testing.T) {
	...
	mock.ExpectQuery(regexp.QuoteMeta(`
		...
		LIMIT $2
	`)).
		WithArgs("usr_replace", adminRBACListLimit).
		WillReturnRows(sqlmock.NewRows([]string{"role_code", "product"}).
	...
	mock.ExpectQuery(regexp.QuoteMeta(`
		...
		FOR UPDATE
	`)).
		WithArgs(pq.Array([]string{"role_a"})).
		WillReturnRows(sqlmock.NewRows([]string{"code", "product", "built_in"}).
			AddRow("role_a", controlplane.ProductAutoLive, false))
	mock.ExpectQuery(regexp.QuoteMeta(`
		SELECT product
...
func TestPostgresRepositoryListUserAdminRolesFiltersByProductAndKeepsGlobalSuperAdmin(t *testing.T) {
	...
	mock.ExpectQuery(regexp.QuoteMeta(`
		...
		LIMIT $3
	`)).
		WithArgs("usr_list", string(controlplane.ProductAutoLive), adminRBACListLimit).
		WillReturnRows(sqlmock.NewRows([]string{"role_code", "product"}).
			AddRow(controlplane.BuiltinAdminRoleSuperAdmin, nil).
			AddRow("role_a", controlplane.ProductAutoLive))
