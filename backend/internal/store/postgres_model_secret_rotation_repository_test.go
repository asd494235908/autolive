...
func TestPostgresRepositoryRotateModelPoolAccountSecretUsesSharedTransaction(t *testing.T) {
	...
	mock.ExpectExec(regexp.QuoteMeta("INSERT INTO audit_outbox (")).WithArgs(sqlmock.AnyArg(), "audit-request:req-rotate-account", sqlmock.AnyArg(), now).WillReturnResult(sqlmock.NewResult(1, 1))
	mock.ExpectCommit()
