...
func TestPostgresRepositoryRecordAuditUsesNormalizedAppend(t *testing.T) {
	...
	mock.ExpectBegin()
	mock.ExpectExec(regexp.QuoteMeta("INSERT INTO audit_logs (")).WithArgs(sqlmock.AnyArg(), "usr_1", "dev_1", "model.lease.release", "model_lease", "lease_1", "req-1", "success", 200, "", now).WillReturnResult(sqlmock.NewResult(1, 1))
	mock.ExpectCommit()

	err = repository.RecordAudit(context.Background(), controlplane.AuditLogInput{ActorUserID: "usr_1", DeviceID: "dev_1", Action: "model.lease.release", TargetType: "model_lease", TargetID: "lease_1", RequestID: "req-1", Outcome: "success", StatusCode: 200})
	if err != nil {
