...
## 2026-09-07 当前失败复核与修复

- 用户提供的 `rtmp\://192.168.10.22:1935/live/demo1` 若原样进入输入框，会在客户端地址校验阶段被拒绝；错误提示已明确说明必须填写 `rtmp://` 或 `rtmps://`，不能包含反斜杠。该路径不会启动 FFmpeg，也不会在 ZLMediaKit 留下连接日志。
- v100 目录包的 `GpAutoLive.Media.dll`、`GpAutoLive.Contracts.dll` 与当前源码构建哈希不一致；旧包仍包含 `h264_mf` 的 `-quality speed`，而本机 `h264_mf` 探测虽成功，运行时会因此在 RTMP 连接前退出。当前命令构造已移除该参数，并保留最小回归测试。
- 使用现有媒体运行时直接以 `h264_mf` 向 `192.168.10.22:1935` 发布，服务器日志确认握手成功并注册 H.264 `1280×720@30fps` 与 AAC `48000Hz/2ch`，进程退出码为 `0`；本次未修改服务器配置。该证据将问题收敛到旧客户端包/输入地址，不支持把服务器判定为故障。
- 已生成 `artifacts/csharp-windows-controller-20260907-v101`，只覆盖 v100 的本地目录候选副本中的 C# 根程序集，未覆盖 v100、未安装到 `Program Files`；包核验通过，未签名/法律待审边界保持不变。
