...
> 2026-09-10 抖音登录/房间分离补充：实施中。目标为同次运行内免扫码重连、同房重复连接幂等、清晰区分认证失效与本地通信/辅助进程异常；不保存跨重启凭据。本轮真实发送仅授权一次中文问号 `？`，自动回应关闭；结果见 [修复记录](./2026-09-10-抖音登录与房间连接分离修复.md)。

> 2026-09-10 抖音单条手动发送：弹幕窗口已加入文本输入/发送按钮，独立于自动回应开关及回复池，复用宿主限频/互斥/代际校验；未知结果不重试，accepted 不冒充本人回显。93 项相关 C# 定向测试、本地开发输出构建通过并启动。实际发送尚未执行，安装包未更新；文件、命令和页面工具限制见 [专项记录](./2026-09-09-抖音实时弹幕窗口实施方案.md)。

> 2026-09-10 v102 云测试安装包：当前 C# 源码已本地发布并生成 `GpAutoLive-Setup-v102-TEST-UNSIGNED-LEGAL-REVIEW.exe`，含 PortAudio 常显设备选择及近期客户端修改。最终全套 897 通过、15 项实机条件跳过、0 失败；完整 Release 构建 0 警告/错误，目录包、publish 哈希与 EXE 元数据校验通过。保留 CloudTest、未签名和法律待审边界；未集成虚拟摄像头驱动及抖音 Conda 环境安装。现有客户端运行导致目录包启动冒烟未执行，真实安装/卸载和换机业务待验收。产物、哈希、命令与历史失败修复见 [v102 打包记录](./2026-09-10-v102云测试EXE打包记录.md)。

> 2026-09-09 抖音实时弹幕：真实 WPF 扫码、持续消息显示与小窗口关闭重开已验证。窗口显示最近 500 条昵称、正文和时间，只观看允许空回复池，已连接监听不再受 300 秒扫码超时或累计 stdout 256 KiB 限制。自动回应发送、跨账号/网络矩阵及安装包仍未验收，详见 [实施记录](./2026-09-09-抖音实时弹幕窗口实施方案.md)。

> 2026-09-08 视频换源保帧：代码已接入，手动/暂停切换、EOF 回绕、单项循环、播放池替换和文件失效通过真实 WPF/WGC 夹具检查；具体最终命令、像素样本和未验证边界以 [本次修复记录](./2026-09-08-视频切换保帧修复.md) 为准。切换时短暂保留旧帧，继续使用单 mpv，加载失败不暴露播放器黑底。尚未重新发布安装包。

> 2026-09-08 C# 媒体生命周期与 RTMP 时间轴修复：解码器/原生音频/mpv 停止失败保留 owner 并明确失败；进程释放须确认 Windows 内核句柄真实退出，修复取消后文件仍占用；WPF 关闭异步 Join，麦克风迟滞、文件插话跨候选、RTMP 首 PCM 源锚点、暂停/seek 声音重建已修复。最终 C# 全套 810 通过、3 跳过、0 失败，本地 Release 构建 0 警告/错误；真实 CPU4/GPU83、声音处理、WPF seek、10 轮文件插话/虚拟输入、11.024 秒锚点颜色/音调回读通过。RTMP 最终两轮三轨接收/解码/正常退出通过，服务端 6 次即时注销；反向 AV→video 也通过，未修改服务器。真人听感、实体麦克风、逐采样插话一致性、长稳及跨设备矩阵未验收。本轮未重打安装包。文件、命令、历史失败与最终证据见 [修复验收记录](./2026-09-08-CSharp媒体链路修复验收记录.md)。

> 2026-09-07 RTMP 失败修复复核：定位到两条独立的启动前失败路径。用户若把地址写成 `rtmp\://...`，客户端会在 FFmpeg 启动前拒绝反斜杠；当前提示已改为明确要求使用 `rtmp://` 或 `rtmps://`。另外，v100 目录包中的 Media/Contracts DLL 与当前源码哈希不一致，旧包仍可能为 `h264_mf` 拼接不受支持的 `-quality speed`，从而在连接 ZLMediaKit 前退出；当前源码已移除该参数。使用已安装媒体运行时的 `h264_mf` 向 `rtmp://192.168.10.22:1935/live/diagnose` 短推验证退出码为 `0`，服务器日志确认已注册 H.264 `1280×720@30fps` 与 AAC `48000Hz/2ch`，说明服务器和网络可用。新 v101 本地目录候选已生成并通过包核验；未覆盖 v100、未自动安装，签名和法律材料仍按内部测试边界处理。Contracts `30/30`、Media `142/142`、Windows RTMP `32/32` 定向测试通过。

> 2026-09-07 RTMP 与麦克风入口修复：`h264_mf` 不再拼接 FFmpeg 不接受的字符串参数 `-quality speed`；本机使用 `h264_mf` 向 `rtmp://192.168.10.22:1935/live/demo` 短推流退出码 `0`，RTMP 命令构造回归测试 `8/8` 通过。麦克风测试入口改为显式 WPF 事件，并与启用门控统一使用“已授权、已选 PortAudio 输入、已有未关闭 FinalPcmBus、未处于并发命令”门禁；未播放带声音媒体时显示下一步操作提示，相关 App 测试 `6/6` 通过。麦克风仍只做本地 RMS/VAD 门控，不宣称 AEC/降噪/AGC。虚拟摄像头本轮未绕过门禁：当前包仍缺少签名 AkVirtualCamera/sidecar/DirectShow 制品和 PnP 注册，保持“不可用/待补齐外部发布制品”。

> 2026-09-07 v100 云测试安装候选：视频低感知参数两位小数显示修复已进入新的本地 Release 目录包和 NSIS 单文件安装程序，未覆盖 v99；固定 `CloudTest/cloud-test-v1` 与 `http://101.96.208.132:9090`。完整本地解决方案测试 `733` 项通过、`3` 项按显式实机条件跳过、`0` 失败；v100 目录包主 DLL 与本轮 publish SHA-256 一致，BAML 已确认包含两位小数格式，包内 GPU83 最终窗口像素验证 `1/1` 通过。安装器产品版本 `v100 / 0.1.100.0`，大小 `169,559,694` bytes，SHA-256 `5B88129C304C50202B358F317EA01E58E2246E3A1F213241721C34F2B8AD97BD`；目录包启动、窗口就绪、优雅关闭和零残留进程冒烟通过。该候选仍未签名、mpv/FFmpeg 法律待审，也未执行管理员/干净机真实安装卸载，只允许内部云测试。

> 2026-09-07 打包后视频参数显示精度修复：v99 安装候选仍把亮度、对比度、饱和度、色相和锐度按整数显示，而生产自动周期已收窄到小于 `0.5` 的低感知变化，因此界面长期显示为 `0% / 100% / 100% / 0° / 0%`，造成“参数没有变换”的假象。当前五项绑定统一保留两位小数，不扩大生产随机范围、不修改 GPU83/CPU4 算法或周期时钟。v99 包内 mpv、`gpu83.hook`、媒体清单和业务 DLL 核验未发现视频资源漏打包；使用 v99 目录运行包的真实 GPU83 夹具已完成 shader 参数写入、随机种子更新和回读，最终像素变化夹具 `1/1` 通过，长流程夹具仅在后续 EOF 状态观察阶段因取消失败，不能记作整项通过。新增 WPF 回归测试保证低感知值不会再被格式化成默认整数；修复必须以新版本重新发布，v99 不覆盖。

> 2026-09-07 视频参数窄宽布局修复：截图中的 `GPU·GPU83`、源素材名和八项视频参数重叠来自卡片内标签与数值共用同一层 Grid，不是模式判断或参数绑定错误。当前四张摘要卡与八张参数卡统一改为“标签在上、数值在下”的紧凑两行布局，高度由 `38` 调整为 `50`；源素材继续省略显示并可通过 Tooltip 查看完整名称。`GPU·GPU83/CPU·CPU4/未处理·Original` 仍来自 mpv 已确认的真实会话模式。1280×800 布局结构与坐标定向测试、相关 App 测试 `9/9` 通过，1586×992 授权态夹具已重新生成并目视确认；真实 DPI、多屏和安装包内页面仍待用户复验。

> 2026-09-07 v99 云测试安装候选：已把上述视频参数窄宽布局修复纳入新的本地 Release 目录包和 NSIS 单文件安装程序，继续固定 `CloudTest/cloud-test-v1` 与 `http://101.96.208.132:9090`。目录包根文件、GPU/WinRT/媒体清单、大小、SHA-256 和符号清单核验通过；安装器产品版本 `v99 / 0.1.99.0`，大小 `169,569,304` bytes（161.71 MiB），SHA-256 `7A903E889447F911071C47CC0D41F13A2BA9780168687F3B49D62757D74ED1D1`；目录包启动、窗口就绪、优雅关闭和零残留进程冒烟通过。该候选仍未签名、mpv/FFmpeg 法律待审，也未执行管理员/干净机真实安装卸载，因此只能内部测试。

> 2026-09-07 v98 云测试 NSIS 候选：`CloudTest` 口味在应用目录写入 `cloud-test-v1`，双击安装后的客户端无需外部环境变量即可固定连接 `http://101.96.208.132:9090`；不写 Windows 全局环境变量，测试 Refresh Token、待撤销 Token 和设备身份使用独立 `GpAutoLive.CSharp.Windows.Test/` 凭据前缀。`Production` 口味写入 `production-v1`，拒绝继承遗留的 `test/development` 配置及固定测试 HTTP，只接受显式 HTTPS 地址。v96 已修复 staging 当前目录导致的激活失败，v98 保留该修复；NSIS 3.12 将完整应用与微软签名 `.NET 10.0.11 Windows Desktop Runtime x64` 压缩为 161.70 MiB。mpv/FFmpeg 法律审核、内部 PE/外层签名及管理员/干净机安装卸载仍未完成，所以产物只能标记为 `TEST-UNSIGNED-LEGAL-REVIEW`。
