...
> 2026-09-07 修复覆盖：Armed 使用启动阈值，Speaking/Hangover 使用停止阈值维持迟滞；默认 −42 dB 启动后，持续 −45 dB 不再被截断，跌破 −48 dB 并达到 250 ms 才退出，零挂起和稀疏观测同样按截止时间处理。控制器停止共享 2 秒等待预算，未 Join 的观察任务、取消源及输入 owner 保留供重试，禁止新会话覆盖；重复 Dispose 幂等，未完成回收时明确失败。原生输入输出预算与缓存快照按 C4 的本轮覆盖执行，不能宣称能安全强杀驱动。麦克风 PCM 已通过现有 bridge 进入最终 PCM overlay；下文早期“未接入混音”仅记录历史，真人声音、AEC/降噪/AGC、设备拔插和长稳仍待实际验收。
