...
> 2026-09-07 导入保留修复：主页“添加媒体”文件选择入口统一按 `Append` 提交，与系统拖放一致；已有播放池条目只保留内存引用并继续存在，不再因普通文件选择被 `ReplaceAll` 覆盖。“移除/清空/整批替换”只修改播放池引用，不删除用户源文件；仅应用自行创建且可证明归属的临时缩略图等文件允许由对应所有者清理。
