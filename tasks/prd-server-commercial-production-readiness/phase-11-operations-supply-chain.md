...
用可重复的保留清理、备份恢复、指标/追踪、容量告警、CI 签名/Provenance 和部署前验证证明多产品服务端可生产运行。

## 主 PRD 上下文

- 目标：G-6、G-8～G-10
- 成功标准：SC-6、SC-8～SC-11
- 需求：FR-25～FR-28、FR-34～FR-44，NFR-2、NFR-4～NFR-14
- 场景：所有场景的生产保障

## 阶段发现门禁

- [ ] 重读现有 retention scheduler/metrics、`/livez`/`/readyz`、优雅关闭、CI 和发布脚本。
- [ ] 盘点所有新表/对象的产品归属、分类、TTL、归档、法务冻结和容量增长率。
...
- 全数据集保留/清理/归档、每产品容量/错误预算告警和数据字典。
- PostgreSQL + 对象存储加密备份、隔离恢复、产品关系/席位核对、RPO/RTO 证据和 Runbook。
- Prometheus 业务/运维指标和 OpenTelemetry OTLP traces。
- CI 制品签名、SBOM/Provenance、签名验证、发布清单、迁移/回滚与服务器无构建门禁。
- React 运维视图：Worker 积压/死信、备份新鲜度/恢复演练摘要、每产品存储容量、发布验证与告警链接。
...
- [ ] 为每个数据集建立 owner、product 归属、分类、TTL、清理键、索引、批次、冻结/归档和容量告警矩阵。
- [ ] 扩展 Retention Scheduler 而非建立无主时器，使用有界批次、`SKIP LOCKED`、超时、取消、数据集/产品隔离和优雅关闭。
- [ ] 建立 PostgreSQL 定时加密备份、完整性检查、异地/跨失效域保留和过期删除。
- [ ] 建立对象存储版本/对象锁或等价保留、清单备份和孤儿/丢失对象核对。
- [ ] 至少每季度恢复到隔离环境，执行迁移、数量、产品关系、跨产品引用、席位占用和密文可读性核对，记录实际 RPO/RTO。
- [ ] 扩展 Prometheus：订单/支付结果、Worker lag/retry/dead letter、错误报告、对象存储、备份/恢复、发布验证；只用固定低基数标签。
- [ ] 建立每产品容量/错误预算与公平限额，证明一个产品的洪峰不会拖垮另一个产品。
- [ ] 接入 OpenTelemetry Go tracing 与 OTLP exporter，覆盖 HTTP、DB、Worker、支付/邮件/对象存储出站调用和 trace context 传播。
- [ ] 建立告警/Runbook：支付对账积压、死信、连接池、保留失败、备份过期、恢复超时、跨产品完整性、制品验签失败。
...
- [ ] 将只读运维视图绑定 `operations.read`，将重试/解除冻结/发布等操作绑定 `operations.manage` 或对应领域权限。
- [ ] 产品 kill switch、最低版本和强制升级属于高风险操作，要求确认、幂等、理由和审计。
- [ ] React 运维页只显示低基数摘要和有界运维记录，通过明确开放链接跳转外部指标/trace 系统，不在管理端重造监控平台。

## 验证策略

在自动测试之外，本阶段必须有隔离恢复、跨产品完整性核对、告警演练、trace 串联、签名策略负测试和服务器只加载制品的环境证据。

## 验证清单

- [ ] 保留 Worker 大数据批次、锁竞争、取消、产品公平性、部分数据集失败和 SIGTERM。
- [ ] 备份缺失/损坏/密钥不可用、隔离恢复、产品关系/席位/对象核对与实际 RPO/RTO 记录。
...
- [ ] 实际恢复演练证明 RPO≤24h、RTO≤4h，产品关系、席位和跨产品约束通过核对。
...
- 2026-08-22：顺延为 Phase 11，纳入产品公平性、跨产品恢复核对、客户端报告保留和受权限保护的 React 运维摘要页。
