# Phase 6：微信支付

Parent PRD：[PRD：服务端商业化与生产就绪](../prd-server-commercial-production-readiness.md)
状态：Not Started
最后更新：2026-08-22

## 目标

使用微信支付 API v3 官方 Go SDK 接入 Native 支付，完成安全回调、幂等落库、未知结果对账和订阅权益交付。

## 主 PRD 上下文

- 目标：G-2、G-8、G-9
- 成功标准：SC-2、SC-9、SC-10
- 需求：FR-7～FR-11、FR-34～FR-39，NFR-2～NFR-5、NFR-8、NFR-12、NFR-13
- 场景：场景 3

## 阶段发现门禁

- [ ] 用户确认 Native/JSAPI/App/H5 中的首个支付形态；未变更时按 Native
- [ ] 重读微信支付 API v3 官方 SDK、回调验签/解密、签名探测、公钥/平台证书轮换和订单查询文档
- [ ] 确认商户号、APPID、回调 HTTPS 域名、API v3 密钥和私钥的 Secret Store/外部密钥边界
- [ ] 确认 Phase 4 订单/订阅状态机和 Phase 5 管理查询/页面已完成且不需要退款范围
- [ ] 确认订单、回调履约、订阅和席位均绑定同一 product，支付通知不能改变产品归属
- [ ] 确认小额真实支付验收账号与资金处理责任人

## 范围

### 包含

- Native 预支付下单、查询、关闭、回调验签/解密与支付尝试状态。
- 通知幂等、金额/主体校验、支付/订单/订阅原子履约、对账 Worker、下游通知 Outbox 和运维查询。
- 支付 Secret、限流、超时、审计、指标和 trace 脱敏。
- React 支付详情、对账队列/结果和受权限保护的手工对账操作。

### 不包含

- 不手写 API v3 签名协议，不保存原始支付通知到普通日志/审计。
- 不实施退款、分账、优惠、发票或其他支付渠道。

## 实施清单

- [ ] 引入并锁定微信支付 API v3 官方 Go SDK，记录版本、许可证、安全记录和移除路径。
- [ ] 建立 WeChatPayAdapter，只暴露 create/query/close/verify-and-decrypt 领域需要的最小边界。
- [ ] 新增 payment attempts/notifications/reconciliation jobs 前向迁移与唯一约束。
- [ ] 下单采用本地订单短事务 → 事务外微信调用 → 短事务写入尝试结果，结果未知进入对账。
- [ ] 回调读取受限原始 body，在解析业务 JSON 前使用 `Wechatpay-*` 头验签/解密，快速落库并按官方规则应答。
- [ ] 校验通知 ID、交易号、商户订单号、金额、CNY、APPID和商户号，不一致时拒绝交付。
- [ ] 履约只使用本地订单已固定的 product；回调正文中的任何扩展字段不得覆盖或跨产品交付。
- [ ] 已验证支付事实、订单终态、订阅权益和下游通知 Outbox 同事务；重复通知返回幂等成功，核心权益不依赖通知 Worker 才生效。
- [ ] 对账 Worker 查询长时间 paying/unknown 订单，使用有界重试与死信。
- [ ] 实现支付密钥/公钥或证书轮换、就绪检查、健康观测和不泄密运维 Runbook。
- [ ] 使用 `payments.read`/`payments.reconcile` 和 `orders.read`/`orders.reconcile` 区分只读与高风险操作，手工对账要求原因、确认和幂等键。
- [ ] 在 React 订单/支付页显示脱敏状态、供应商查单结果和 Worker 积压，不显示密钥、原始通知或完整二维码 URL。

## 验证策略

官方 SDK fixture 与受控假 server 覆盖签名/验签/解密/超时，PostgreSQL 集成覆盖重入/并发/断连，发布前再做小额真实支付、回调和查询验收。

## 验证清单

- [ ] 有效/无效/签名探测、错误 serial/public key、时间戳/随机数/body 篡改和解密失败
- [ ] 重复/并发通知、支付成功后 Worker 崩溃、提交未知和对账恢复
- [ ] 金额、APPID、商户号、订单号任一不一致均不交付
- [ ] 回调不能把订单交付到另一 product，支付/订单管理查询遵守产品范围权限
- [ ] 密钥/原始通知/二维码 URL 不进入日志、metrics、trace、审计或测试快照
- [ ] 小额真实支付、回调、查询和重复通知验收有脱敏证据
- [ ] React typecheck/test/build 和浏览器冒烟覆盖只读、可对账、无权限、提交重试与结果未知

## 退出标准

- [ ] 订单、支付事实和订阅交付在重复/延迟/丢失/重启/未知结果下仍一致
- [ ] 微信密钥轮换、回调验签、对账和运维告警可操作

## 阶段末多轮复核

- [ ] 1. 意图/覆盖；2. 正确性；3. 简化；4. 边界/命名；5. 重复/清理
- [ ] 6. 安全/隐私；7. 性能/容量；8. 验证充分性；9. 后续阶段；10. 主 PRD 同步

## 发现/决策

- 2026-08-21：规划优先使用微信支付官方 Go SDK，默认 Native 支付，本轮未实施。
- 2026-08-22：纳入 React 支付/对账管理页和独立读/对账权限点。
- 2026-08-22：支付事实、订单、订阅和席位固定在同一 product，回调不得改变产品归属。
