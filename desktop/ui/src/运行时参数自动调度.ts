...
const DEFAULT_RUNTIME_VARIATION_PERIOD_MS = 15_000;
...
export const AUDIO_VALUE_PRESETS: readonly AudioValuePreset[] = [
  ...
  { id: 'p10', label: '10. 相位微扰', values: audioValues({ phase_perturbation_percent: 1.8, spectral_perturbation_percent: 0.4 }) },
  { id: 'p11', label: '11. 轻混响', values: audioValues({ reverb_wet_percent: 1.1, dry_wet_percent: 1.2, fade_in_ms: 12, fade_out_ms: 16 }) },
  { id: 'p12', label: '12. 干声收紧', values: audioValues({ reverb_wet_percent: 0.2, dry_wet_percent: 0.3, noise_reduction_percent: 1.2 }) },
  { id: 'p13', label: '13. 轻降噪', values: audioValues({ noise_reduction_percent: 1.6, high_eq_db: -0.15, spectral_perturbation_percent: 0.2 }) },
  { id: 'p14', label: '14. 底噪纹理', values: audioValues({ environment_noise_percent: 0.55, environment_noise_dbfs: -49, ambient_sound_mix_percent: 0.6 }) },
  ...
  { id: 'p18', label: '18. 空间感', values: audioValues({ reverb_wet_percent: 1.3, dry_wet_percent: 1.4, phase_perturbation_percent: 1.2, fade_in_ms: 14, fade_out_ms: 18 }) },
  { id: 'p19', label: '19. 调制组合', values: audioValues({ vibrato_frequency_hz: 5.6, vibrato_depth_percent: 0.3, phase_perturbation_percent: 1.5, spectral_perturbation_percent: 0.7 }) },
  { id: 'p20', label: '20. 综合微扰', values: audioValues({ pitch_shift_semitones: -0.02, input_gain_db: 0.12, output_gain_db: -0.05, loudness_adjustment_db: 0.08, low_eq_db: 0.18, mid_eq_db: -0.12, high_eq_db: 0.22, filter_q: 1.06, phase_perturbation_percent: 1.1, vibrato_frequency_hz: 5.2, vibrato_depth_percent: 0.18, reverb_wet_percent: 0.6, noise_reduction_percent: 0.8, environment_noise_percent: 0.25, environment_noise_dbfs: -50, fade_in_ms: 10, fade_out_ms: 12, dry_wet_percent: 0.8, ambient_sound_mix_percent: 0.3, spectral_perturbation_percent: 0.35 }) },
] as const;

export const DEFAULT_AUDIO_VALUE_PRESET_IDS: readonly string[] = AUDIO_VALUE_PRESETS.map((preset) => preset.id);
...
export function sampleSubtleAudioParams(
  ...
  const pool = selectedPresetIds
    .map((id) => AUDIO_VALUE_PRESETS.find((preset) => preset.id === id))
    .filter((preset): preset is AudioValuePreset => Boolean(preset));
  if (pool.length === 0) return audioValues({});
...
export function sampleSubtleVideoParams(random = Math.random): SubtleVideoSample {
  ...
  return {
    ...
    crop_edge_smoothing: roundTo(inRange(0.45, 0.55), 3),
    frame_rate_jitter_percent: roundTo(inRange(0, 0.4), 2),
    frame_rate_perturbation_frequency_hz: roundTo(inRange(0.05, 0.2), 3),
    frame_rate_perturbation_amplitude_fps: roundTo(inRange(0, 0.2), 3),
    frame_inner_perturbation_percent: roundTo(inRange(0, 0.3), 2),
    frame_inter_perturbation_percent: roundTo(inRange(0, 1), 2),
    color_space_conversion_strength_percent: roundTo(inRange(0, 2), 2),
  };
}
