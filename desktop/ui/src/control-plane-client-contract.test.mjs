...
test('桌面端控制面请求遵守 JSON、请求追踪和统一错误契约', async () => {
  ...
  assert.doesNotMatch(source, /loginControlPlane|refreshControlPlane|logoutControlPlane/);
  assert.doesNotMatch(source, /'\/api\/v1\/(?:client\/)?auth\//);
  ...
  assert.match(source, /const testControlPlaneBaseUrl = 'http:\/\/101\.96\.208\.132:9090'/);
  assert.match(source, /testControlPlaneBuild && normalizedBaseUrl === testControlPlaneBaseUrl/);
  assert.match(source, /viteEnvironment\?\.DEV && loopbackHttp/);
  assert.match(source, /must be set for production desktop builds/);
  assert.match(source, /parsed\.protocol !== 'https:'/);
  assert.match(source, /must use HTTPS; development HTTP is limited to loopback and test HTTP to the fixed test origin/);
