...
function archiveWindowsPortable({
  ...
  try {
    ...
    const portaudioReadme = resolveDesktopPath('src-tauri/portaudio/README.md');
    if (isFile(portaudioDll)) {
      cpSync(portaudioDll, join(stagingRoot, 'portaudio_x64.dll'));
    }
    if (isFile(portaudioLicense)) {
      mkdirSync(join(stagingRoot, 'portaudio'), { recursive: true });
      cpSync(portaudioLicense, join(stagingRoot, 'portaudio', 'LICENSE.txt'));
      if (isFile(portaudioReadme)) {
