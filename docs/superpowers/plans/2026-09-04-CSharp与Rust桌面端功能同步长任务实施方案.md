...
- 版本：v1.124
- 日期：2026-09-07
- 本轮真实 C# 黄金路径复验：使用 `D:\xz\8d020eb133350a74bbc4daec1f33bbc1.mp4` 与已校验 `csharp-gpu83-real-v2` 运行包，主窗口导入→FFprobe→首项视频播放→真实 EOF 自动换源 `2/2` 通过；视频声音处理开关重建 FFmpeg/PortAudio 会话 `2/2` 通过；真实 GPU83 最终窗口像素变化 `1/1` 通过。声音滤镜直接输出与原始 PCM 的字节长度和 SHA-256 均不同。未把该证据扩大为人工 UI、目标显卡矩阵、设备拔插、RTMP 远端或长稳通过；不启动、不修改 Rust。
...
- v1.78 本轮收口：底部输出音量滑块接入既有最终 PCM 混音策略，0～100% 经过有限 dB 映射实时作用于 PortAudio 和 RTMP 分支；声音处理开关增加单调版本号，异步 FFmpeg/PortAudio 重建在多次快速切换时拒绝过期请求，避免旧会话覆盖最新状态。最终效果窗口底栏固定为独立布局行，窗口 Loaded、F11 进入/退出全屏和 WGC 顶层 HWND 查询都会刷新呈现布局，保留播放/暂停、停止、关闭、状态、进度和脱敏会话信息；mpv 继续使用视频子 HWND，WGC 继续使用最终效果窗口顶层 HWND。Media 音频混音测试 `135/135`、FinalEffect/声音状态目标测试 `10/10`、格式检查通过、Release x64 构建 `0` 警告/`0` 错误；真实声卡、人工页面、真实 FFmpeg/PortAudio 声音切换夹具、远端输出和长稳仍待验收。没有新增播放器、音频流、依赖或 Rust 改动。
