...
- v1.78 本轮收口：底部输出音量滑块接入既有最终 PCM 混音策略，0～100% 经过有限 dB 映射实时作用于 PortAudio 和 RTMP 分支；声音处理开关增加单调版本号，异步 FFmpeg/PortAudio 重建在多次快速切换时拒绝过期请求，避免旧会话覆盖最新状态。最终效果窗口底栏固定为独立布局行，窗口 Loaded、F11 进入/退出全屏和 WGC 顶层 HWND 查询都会刷新呈现布局，保留播放/暂停、停止、关闭、状态、进度和脱敏会话信息；mpv 继续使用视频子 HWND，WGC 继续使用最终效果窗口顶层 HWND。Media 音频混音测试 `134/134`、FinalEffect/声音状态目标测试 `10/10`、格式检查通过、Release x64 构建 `0` 警告/`0` 错误；真实声卡、人工页面、真实 FFmpeg/PortAudio 声音切换夹具、远端输出和长稳仍待验收。没有新增播放器、音频流、依赖或 Rust 改动。
