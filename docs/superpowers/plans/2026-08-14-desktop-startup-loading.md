...
- Create: `desktop/ui/src/启动加载.tsx` — 轻量 Loading 和顶层 Error Boundary。
- Create: `desktop/ui/src/启动加载.test.mjs` — Loading/Error Boundary 的源契约和行为测试。
- Create: `desktop/ui/src/启动调度.ts` — 可取消的首屏后空闲调度封装。
- Create: `desktop/ui/src/启动调度.test.mjs` — 空闲 API 和定时器回退契约测试。
...
**Files:**
- Create: `desktop/ui/src/启动加载.tsx`
- Test: `desktop/ui/src/启动加载.test.mjs`

- [ ] 先写测试：断言 Loading 默认文案、错误边界错误提示和重载按钮契约。
- [ ] 运行 `pnpm exec node --test src/启动加载.test.mjs`，确认在组件不存在或导出缺失时失败。
...
- Create: `desktop/ui/src/启动入口.test.mjs`

- [ ] 先写 HTML/入口源码契约测试，锁定静态壳、`lazy(() => import('./App'))`、`Suspense` 和 Error Boundary。
- [ ] 运行 `pnpm exec node --test src/启动入口.test.mjs`，确认当前空壳和同步入口使测试失败。
...
- [ ] 将媒体引擎、研究 Worker/状态/默认参数和 speech-to-speech 能力探测放入可取消的首屏后调度；研究状态轮询从调度回调开始。
