...
首选实现使用前端原生 `window.speechSynthesis` 与 `SpeechSynthesisUtterance`，不新增第三方依赖。实际删除旧链路之前，必须先在本项目的 Tauri/WebView2 开发构建中通过系统语音能力门禁；门禁失败时停止删除并重新评审 Windows SAPI/WinRT 适配器，不回退到现有模型链。
...
本方案只精简“固定话术播放”。独立的实时音频幻化、视频处理、普通声音处理、本地研究模式和 FFmpeg 媒体能力不在本次删除范围内。
...
- 不改造实时音频幻化 Worker，不删除该功能真实使用的 `speech_to_speech_adapter.py`、FFmpeg/FFprobe 或公共 Windows 子进程策略。
- 不在本次任务中追求单文件便携 EXE。正式交付包含 NSIS 安装程序 `.exe`，并同时保留“可执行 EXE + 必需的外部运行资源目录 + ZIP”的便携包结构。

## 3. 当前实现与精简依据

当前固定话术链路是：视频参考音频 → Demucs 分离 → faster-whisper 转写 → XTTS-v2 克隆/合成 → WAV 缓存 → Rust 播放状态 → 最终效果窗口替换音轨。它带来了以下与当前需求无关的成本：
...
| `desktop/ui/src/播放控制消息.ts` | 增加固定话术临时静音消息类型、解析和版本校验 |
| `desktop/ui/src/插话播放器.ts` | 删除 `voice_clone` 基础音源和克隆状态依赖；按新的固定话术活动状态协调插话 |
...
| `desktop/ui/src/固定话术播放.test.mjs` | 改写为系统朗读、静音租约、取消与恢复契约测试 |
...
- `desktop/工具/准备语音模型资源.mjs`
- `desktop/工具/准备语音模型资源.test.mjs`
- `desktop/工具/准备语音Worker资源.mjs`
- `desktop/工具/准备语音Worker资源.test.mjs`
- 从 `desktop/工具/生成运行资源发布树.mjs` 及测试删除 `voice-worker` 和 `voice-models` 发布项。
- 从 `desktop/工具/启动桌面开发环境.mjs` 及测试删除固定话术 Worker/模型准备和 `AUTOLIVE_VOICE_CLONE_*` 环境变量。
- 从 `desktop/ui/package.json` 删除语音 Worker/模型准备脚本及其在测试、开发、构建链中的调用。
- 更新 `desktop/工具/轻量桌面打包.test.mjs`，断言便携包不再包含固定话术大资源。
...
- `desktop/worker/speech_to_speech_adapter.py` 及实时音频幻化依赖。
- `desktop/worker/windows_subprocess_policy.py`，因为它仍被实时 Worker 使用。
- FFmpeg、FFprobe 及内部媒体依赖准备/校验链，因为视频处理、普通声音处理和实时音频幻化仍需要它们；这些依赖不作为桌面端独立“运行资源”产品区域展示。
...
- `产品需求文档.md`：把固定话术定义为自动选择本地系统声音朗读，明确无 ASR/分离/克隆和在线语音回退；不改变实时音频幻化范围。
...
`docs/superpowers/specs`、`docs/superpowers/plans` 和 `tasks/prd-voice-clone-current-playback*` 属于历史设计/迁移审计。实施时不把它们改写成当前架构；在入口或文件头加“已废止，仅供历史审计”标记，防止后续开发误用。是否最终物理删除历史文档，作为文档审计后的独立决定，避免同时丢失迁移依据。
...
Windows 打包在仓库现有本地发布流程上做一处明确调整：`desktop/工具/构建桌面产物.mjs` 不再在 Windows 强制 `--no-bundle`，而是调用 Tauri 官方 NSIS bundle；`desktop/工具/归档桌面产物.mjs` 同时归档 NSIS 安装程序和 portable ZIP。预期产物包括：
...
- 实时音频幻化、视频处理、普通声音处理和 FFmpeg 媒体能力未被误删，相关测试继续通过。
...
| 删除误伤实时音频幻化 | 按真实调用搜索逐项复核；保留 `speech_to_speech_adapter.py`、公共子进程策略和 FFmpeg |
...
4. 实时音频幻化保留，固定话术相关 Python/模型链全部删除。
...
- 已删除固定话术的 Rust IPC/状态/Worker、Python 适配器、模型准备脚本、语音依赖和旧契约测试；实时 `speech_to_speech`、FFmpeg、视频/普通声音处理和研究能力保留。
