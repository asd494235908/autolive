...
> 当前版本范围声明（2026-08-30）：本版本不开发实时话术幻化。当前新增三项尚未实施的正式需求：麦克风插话、ZLMediaKit RTMP/RTMPS GPU 直推，以及 AkVirtualCamera GPU 辅助虚拟摄像头输出。RTMP 不捕获窗口；虚拟摄像头是独立本地链，只捕获本应用最终效果 HWND，由 WGC/D3D11 在 GPU 上处理后一次有界回读到 AkVirtualCamera CPU raw 边界，明确不是零拷贝。三项能力均不经过 Go，完成真实门禁前不得声称已接入。
...
- [麦克风插话与插话文件入口实施方案](./docs/superpowers/plans/2026-08-30-麦克风插话与插话文件入口实施方案.md)：当前麦克风插话、PortAudio 全双工、AEC/VAD、数字静音、资源生命周期和实机门禁的唯一实施入口；尚未实施
- [ZLMediaKit RTMP GPU 直推实施方案](./docs/superpowers/plans/2026-08-30-ZLMediaKit-RTMP-GPU直推实施方案.md)：用户输入完整 RTMP/RTMPS 地址、音画轨道选择、直接源处理、GPU 编码降级、最终 PCM 分流、会话生命周期和真实 ZLMediaKit 门禁的唯一实施入口；尚未实施
...
flowchart LR
    ...
    Microphone["本地麦克风插话（待实施）"] --> SpeechDSP["AEC / 降噪 / 门控 VAD"]
...
- 麦克风插话（正式需求·待实现/未接入）：用户显式开始监听后，本地 PortAudio 全双工采集经 AEC、降噪和说话活动门控；说话期间主媒体数字静音，只保留清理后的麦克风声音。音频不录制、不上传、不调用模型，详细边界以专项实施方案为准
...
- 2026-08-30：新增 ZLMediaKit RTMP/RTMPS GPU 直推正式需求和唯一实施方案入口。桌面端直接读取当前活动源、复用 FFmpeg/libplacebo/GPU83 与硬件编码器探测，并从最终 PCM 总线分流声音；用户默认发布音画双轨且可只选一轨。该能力尚未实施，不捕获窗口、不经过 Go、不接入 OBS、平台账号或多平台分发。
- 2026-08-30：新增麦克风插话正式需求和唯一实施方案入口；允许本地 AEC、降噪和只判断有人声/无人声的 VAD 控制主媒体数字静音，同时继续禁止 speech-to-speech、ASR/LLM/TTS、实时变声、话术改写、模型调用、录音和上传。该功能尚未实施，不能标记为已接入。
