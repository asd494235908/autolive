...
>
> 实施状态（2026-08-28）：受管 mpv 持久 IPC、受控 libplacebo hook、GPU83 精确映射和 React→Rust 正式周期入口已落地；旧视频 MSE/period/逐周期 FFmpeg 生产代码与权限已删除，普通声音链保持不变。Phase 1 已实现 D3D11/Vulkan/CPU4 实机矩阵、200 次同 PID 参数更新、`vo-passes` 帧预算和发布材料门禁；最新本机结果为 Vulkan 全矩阵通过、D3D11 1080p 全矩阵通过但 4K 略超预算，且此前 D3D11 1080p50 也曾超预算，精确 4K 输出与许可证材料仍未准入。当前仅 `20/83` 项已验证，生产 CPU4 降级、真实 PortAudio 闭环、mpv EOF/播放池接管和长稳仍未完成。Original 继续承担失败兜底。
>
...
- Worker 内同一暂存产物的内容探测重试与前端周期级恢复相互独立：声音候选最终失败后不重复使用已清理产物，而是在保持当前 N 的前提下按 `1/2/4/8/15s` 最多五次自动重建新候选，第六次连续失败才等待人工重试。视频实时提交与外层 apply 的完成顺序不固定；任一完成路径都必须尝试排空 latest-wins pending，调度互斥只读取同步 in-flight 引用和 Rust processing 快照，不以 React 展示状态为事实源。
...
- 最终效果窗口同时是双周期播放时钟健康的唯一观察者：声音周期和视频周期继续消费当前 `playback_generation + source` 的真实绝对媒体位置，Rust `playing` 只表达期望状态，不能代替位置推进证据。排除暂停、seek、切源和媒体加载宽限后，真实位置连续约 `1500ms` 未推进即发布时钟停滞；周期进度冻结，不使用墙钟补偿。同一源代次最多执行一次有代次保护的重载、复位位置和续播恢复；失败必须把时钟投影改为明确停滞/失败并在主窗口展示原因，同时保留 Rust 用户播放意图并继续本地续播尝试，不得反向提交暂停。
- 播放控制命令与媒体引擎事件分层：主页显式“播放/暂停/继续/停止”是 Rust 播放状态的唯一用户意图入口；mpv 内部缓冲、降级和声音槽 pause/load 不能反向提交用户暂停。
- 条目结束边界由 mpv 真实 EOF 事件单飞提交；后端按 `(source_media_pool 当前条目, source_media_index, playback_generation)` 幂等确认下一项。换源递增 generation、建立新同步 epoch，并清理旧声音候选；单项池保持 generation 并递增池循环轮次。
- 仓库中可能存在早期媒体任务、版本计划和阶段 Worker 的历史实现；它们不属于当前架构入口，新功能不得依赖或扩展，代码删除需另行执行迁移清理。
- 本版本不连接 speech-to-speech、VAD/ASR/LLM/TTS，不申请实时话术模型租约，不生成候选话术音轨，也不执行 `realtime_variant` 换轨；相关代码、字段和接口只作历史兼容审计或后续版本预留。普通声音处理统一写入 N/N+1 临时媒体文件的音轨；高质量音高、播放速度、共振峰偏移、MFCC 偏移、MFCC 维度、SNR 浮动、频谱盲区、干湿比和环境声混合参加普通声音周期随机化，桌面 UI 只展示当前值。
- 媒体处理仅在明确启用对应哈希链路时对源文件或产物计算完整字节流 SHA-256；旧阶段报告不属于当前播放契约，正式参数调度所需随机种子仅作为内部可复现信息。
- 视频效果由 mpv 解码、`gpu-next/libplacebo` 实时渲染和呈现，不在周期边界编码。FFmpeg/FFprobe 继续服务于导入探测、声音候选和兼容准备，不用于 RTMP/OBS 推流。
