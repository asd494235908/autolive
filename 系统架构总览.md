...
- [douyin-desktop 接入 autoLive 多产品控制面 PRD](./tasks/prd-douyin-desktop-control-plane-integration.md)：产品硬隔离、Profile 新鲜度、模型认证、固定 OpenAPI 制品与跨仓库 E2E；当前未实施
...
autoLive 计划以单 PostgreSQL 硬逻辑隔离同时承载 `autolive` 与 `douyin_desktop`：全局 `users` 复用身份，`user_products` 管理产品准入，客户端会话绑定单一 product；设备、激活码、租约、用量、审计以及后续商业/配置/制品资源的唯一约束、引用和幂等均包含产品。普通管理员按产品角色授权，筛选参数只能缩小范围，只有内建 `super_admin` 默认跨产品。

Profile 计划由服务端返回时间、状态、6 小时建议复核、24 小时签名离线硬截止和权益 revision。douyin-desktop 首版本地 BYOK；供应商不能提供可撤销、受限、绑定租约的短期凭证时，不开放托管直连。OpenAPI 按 autoLive commit/digest 发布不可变三件套，并用预构建服务/镜像和隔离 PostgreSQL 做跨仓库 E2E。详细设计与四阶段计划见 [`tasks/prd-douyin-desktop-control-plane-integration.md`](./tasks/prd-douyin-desktop-control-plane-integration.md)。
