...
> C# 虚拟摄像头更新（2026-09-09，实施中）：源视频分辨率 → WGC 最终效果捕获 → D3D11 shader 按源尺寸转换 YUY2 → 有界回读 → v2 帧协议 → 独立 AkVirtualCamera sidecar。尺寸不再固定 720p；同尺寸切源复用，不同尺寸重建。测试组件通过显式开发开关和全组件 SHA-256 清单放行未签名，生产强签名；DirectShow 软件设备通过 COM 枚举检查。旧 Rust 固定规格和 Video Processor 描述仅作为历史基线。当前边界及真实验收见 [实施记录](./desktop-csharp-windows/docs/2026-09-09-虚拟摄像头源分辨率与测试组件实施记录.md)。

> C# Windows NSIS 边界（2026-09-07）：C# WPF/SAPI 客户端不依赖 WebView2，正式离线安装器使用 NSIS 携带微软签名的 `.NET 10 Windows Desktop Runtime x64` 与完整媒体/GPU/WinRT 运行库。安装器采用同卷 staging/rollback 切换、运行中拒绝、开始菜单/桌面快捷方式和标准卸载登记；卸载只删除安装器拥有的程序文件，不删除用户配置、凭据或媒体。本文后续 WebView2 offlineInstaller 只描述 Rust/Tauri 历史包。没有有效 Authenticode、发布法律审核和干净 Windows 10/11 实机证据时，NSIS 只能作为内部候选。

> C# 控制面包口味（2026-09-07）：测试与生产安装包分别写入 `cloud-test-v1`/`production-v1` 应用内标记。测试口味固定唯一 HTTP 云测试端点并隔离 Credential Manager 前缀；生产口味禁用测试 HTTP，仅接受显式 HTTPS。地址配置不进入 Windows 全局环境，账号和 Token 不进入安装包。
